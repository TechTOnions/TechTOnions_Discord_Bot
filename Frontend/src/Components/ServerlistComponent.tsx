  ...
  return (
    <div className="w-[80%]">
      <div
        style={
          {
            "--image-url": `url(${data.icon ? data.iconURL : null})`,
      ...
      <div className="flex items-center justify-between w-full my-4">
        ...
        {data.isPresent ? (
          <ButtonGo server_id={data.id} />
        ) : (
          <ButtonSetup server_id={data.id} />
        )}
      </div>
    </div>
  );
...
const ButtonGo = ({ server_id }: { server_id?: string }) => {
  ...
  return (
    <button
      onClick={handleClick}
      className="w-24 h-10 text-center duration-300 ease-in-out rounded-md cursor-pointer hover:bg-sidebarHead transiontion bg-gradient-to-r from-cyan-500 to-blue-500 "
    >
      Go
    </button>
  );
...
const ButtonSetup = ({ server_id }: { server_id?: string }) => {
  const handleClick = () => {
    window.open(
      `https://discord.com/oauth2/authorize?client_id=1075305629641621504&permissions=8&scope=bot&guild_id=${server_id}`,
      "_blank",
  ...
  return (
    <button
      onClick={handleClick}
      className="w-24 h-10 text-center duration-300 ease-in-out rounded-md cursor-pointer hover:bg-sidebarHead transiontion bg-gradient-to-r from-emerald-500 to-green-500 "
    >
      Setup
    </button>
  );
